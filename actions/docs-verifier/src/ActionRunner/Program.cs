...
static bool IsRedirectableFile(PullRequestFile file)
{
    ...
    return !isDeletedToc && IsYmlOrMarkdownFile(deletedFileName) && !IsInWhatsNewDirectory(deletedFileName) &&
        OpenPublishingConfigurationReader.GetDocfxConfigurations().Any(reader => reader.GetMatchers().Any(matcher => matcher.Match(file.PreviousFileName)?.HasMatches == true));
}
